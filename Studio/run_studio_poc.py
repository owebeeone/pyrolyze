...
SOURCE_PATH = Path(__file__).with_name("studio_poc.py")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the Studio prototype with current PyRolyze capabilities."
    )
    parser.add_argument(
        "--root",
        type=str,
    ...
    parser.add_argument(
        "--smoke",
        action="store_true",
        help="Run a short offscreen smoke cycle and exit automatically.",
    )
    return parser
...
def _load_component() -> Any:
    ...
    namespace = load_transformed_namespace(
        source,
        module_name="Studio.studio_poc",
        filename=str(SOURCE_PATH),
    )
    return namespace["studio_poc_app"]
