...
class ContextBaseStateMgr(StateMgrBase):
    def __init__(
        ...
        owner: Any,
        *,
        render_context_state_mgr: Any | None = None,
        render_context: Any | None = None,
        **kwargs: Any,
    ) -> None:
        super().__init__(owner, **kwargs)
        self._generation_tracker_key = type(owner)._generation_tracker_key_const
        self._context_kind = getattr(type(owner), "_context_kind", ContextKind.SLOT)
        self._pass_scope_handle_cls = type(owner)._pass_scope_handle_cls
        self._owner_type_name = type(owner).__name__
        if render_context_state_mgr is not None:
            self._render_context_state_mgr = render_context_state_mgr
        elif render_context is not None and hasattr(render_context, "_state_mgr"):
            self._render_context_state_mgr = render_context._state_mgr
        else:
            self._render_context_state_mgr = None
    ...
    def end_pass(self) -> None:
        if not self._scope_active:
            raise RuntimeError("scope is not active")
        unseen_slots = [slot_id for slot_id, child_state_mgr in self._children.items() if not child_state_mgr._seen_in_pass]
    ...
    def rollback_pass(self) -> None:
        ...
        for slot_id, child_state_mgr in list(self._children.items()):
            ...
            child_state_mgr._invoke_dirty = self._pass_child_dirty.get(slot_id, child_state_mgr._invoke_dirty)
