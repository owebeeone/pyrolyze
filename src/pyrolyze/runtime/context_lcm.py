...
class _SlotCallSlotState:
    ...
    last_args: tuple[Any, ...] = local_store(default_factory=tuple)
    last_kwargs: tuple[tuple[str, Any], ...] = local_store(default_factory=tuple)
    binding: SlotCallBinding | None = local_store(default=None)
    site_metadata: tuple[RuntimeSiteMetadata[Any], ...] = local_store(default_factory=tuple)
...
class _ComponentCallSlotState:
    ...
    last_bound_receiver: object = local_store(default=_BOUND_METHOD_SELF_MISSING)
    last_args: tuple[Any, ...] = local_store(default_factory=tuple)
    ...
    last_dirty_state: DirtyStateContext | None = local_store(default=None)
    pending_dirty_state: DirtyStateContext | None = local_store(default=None)
    ...
    site_metadata: tuple[RuntimeSiteMetadata[Any], ...] = local_store(default_factory=tuple)
    pass_owned_event_handler_order: tuple[SlotId, ...] = local_store(default_factory=tuple)
...
class _AppContextOverrideSlotState:
    ...
    committed_lookup: AppContextLookup = local_store(default_factory=_empty_authored_app_context_lookup)
    pass_committed_values: tuple[Any, ...] = local_store(default_factory=tuple)
    pass_committed_lookup: AppContextLookup = local_store(default_factory=_empty_authored_app_context_lookup)
    pending_values: tuple[Any, ...] = local_store(default_factory=tuple)
    pending_lookup: AppContextLookup = local_store(default_factory=_empty_authored_app_context_lookup)
...
class ContainerSlotContext(_LifecycleSlotMixin, ContainerSlotContext):
    ...
    def _begin_scope_pass(self) -> None:
        self._lcm_txm.begin()
        ContextBase._begin_scope_pass(self)

    def _commit_scope_pass(self) -> None:
        try:
            ContextBase._commit_scope_pass(self)
            self._lcm_txm.commit()
        finally:
            self._lcm_sync()
    ...
    def _rollback_scope_pass(self) -> None:
        try:
            ContextBase._rollback_scope_pass(self)
        finally:
            self._lcm_txm.rollback()
            self._lcm_sync()
...
class SlotCallSlotContext(_LifecycleSlotMixin, SlotCallSlotContext):
    ...
    def evaluate(
        ...
        *,
        ...
        if should_invoke:
            next_result = call_with_optional_runtime_context(
                prepared,
                cache_attr_name="_pyrolyze_slot_runtime_ctx_param",
                runtime_context_annotation=SlotRuntimeContext,
                runtime_context_factory=lambda: SlotRuntimeContext(self),
    ...
    def _build_committed_ui(self) -> tuple[object, ...]:
        ...
        if isinstance(binding, PyrolyzeMountAdvertisementBinding):
            ...
            if advertisement is None:
                return ()
            return (advertisement,)
        return ContextBase._build_committed_ui(self)
...
class DirectiveSlotContext(SlotCallSlotContext):
    ...
    _lcm_fields = SlotCallSlotContext._lcm_fields + (
        "committed_selectors",
        "_pass_committed_selectors",
    )
    _lcm_field_map = {
        **SlotCallSlotContext._lcm_field_map,
        "_pass_committed_selectors": "pass_committed_selectors",
    }
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            ...
            _DirectiveSlotState(
                ...
                site_metadata=self.site_metadata,
                runtime_locals=self._runtime_locals,
                committed_selectors=self.committed_selectors,
                pass_committed_selectors=self._pass_committed_selectors,
            ),
        )
        self._lcm_sync()
    ...
    def _begin_scope_pass(self) -> None:
        self._pass_committed_selectors = self.committed_selectors
        SlotCallSlotContext._begin_scope_pass(self)

    def _commit_scope_pass(self) -> None:
        self.committed_selectors = self.pending_selectors()
        SlotCallSlotContext._commit_scope_pass(self)
        self._pass_committed_selectors = ()

    def _rollback_scope_pass(self) -> None:
        SlotCallSlotContext._rollback_scope_pass(self)
        self.committed_selectors = self._pass_committed_selectors
        self._pass_committed_selectors = ()
    ...
    def invoke(
        ...
        *,
        ...
        try:
            ...
            if dirty_state is None:
                normalized_args = tuple(_bind_pending_event_plain_value(self, _unwrap(arg)[0]) for arg in args)
                normalized_kwargs = {
                    key: _bind_pending_event_plain_value(self, _unwrap(value)[0])
                    for key, value in kwargs.items()
                }
                ...
                self.last_plain_args = tuple(_bind_pending_event_plain_value(self, _unwrap(arg)[0]) for arg in args)
                self.last_plain_kwargs = {
                    key: _bind_pending_event_plain_value(self, _unwrap(value)[0])
                    for key, value in kwargs.items()
                }
...
class AppContextOverrideSlotContext(_LifecycleSlotMixin, AppContextOverrideSlotContext):
    ...
    def _effective_authored_app_context_lookup(self) -> AppContextLookup:
        ...
        if self.declared_keys:
            return self._committed_lookup
        return ContextBase._effective_authored_app_context_lookup(self)
