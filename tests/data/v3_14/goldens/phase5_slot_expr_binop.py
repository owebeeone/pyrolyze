...
__pyr_slot_3 = __pyr_SlotId(__pyr_module_id, 3, line_no=22, is_top_level=True)
...
def __pyr_panel(__pyr_ctx, __pyr_dirty_state, x: int, y: int):
    with __pyr_ctx.pass_scope():
        ...
        value = __pyr_ctx.slot_expr(__pyr_slot_1, lambda v1, v2: v1.eval() + v2.eval(), lambda v1, v2: v1.dirty() or v2.dirty()).slot_call('v1', __pyr_LiteralFunctionProvider(slot_fa), lambda: __pyr_slot_params(x, y), lambda: __pyr_slot_params_dirt(__pyr_dm.bind.x, __pyr_dm.bind.y), slot_id=__pyr_slot_2).slot_call('v2', __pyr_LiteralFunctionProvider(slot_fb), lambda v1: __pyr_slot_params(1, 2), lambda v1: __pyr_slot_params_dirt(False, False), slot_id=__pyr_slot_3).apply_dirt_sink(__pyr_dm).evaluate('value')
        record(value)
