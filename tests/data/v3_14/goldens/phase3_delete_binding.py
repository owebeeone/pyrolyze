...
def __pyr_greeting(__pyr_ctx, __pyr_dirty_state, name: str):
    with __pyr_ctx.pass_scope():
        ...
        title = __pyr_ctx.slot_expr(lambda v1: v1.eval(), lambda v1: v1.dirty()).slot_call('v1', __pyr_LiteralFunctionProvider(format_title), lambda: __pyr_slot_params(name), lambda: __pyr_slot_params_dirt(__pyr_dm.bind.name), slot_id=__pyr_slot_1).apply_dirt_sink(__pyr_dm).evaluate('title')
        del title
